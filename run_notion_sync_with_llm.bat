...
if errorlevel 1 (
    ...
    echo [%TIMESTAMP%] ERROR: Ollama not found >> %LOG_FILE%
    pause
    exit /b 1
)

REM Check if notion-optimizer model exists
echo [%TIMESTAMP%] Checking notion-optimizer model >> %LOG_FILE%
ollama list | findstr "notion-optimizer" > nul
if errorlevel 1 (
    echo notion-optimizer model not found. Creating model...
    echo [%TIMESTAMP%] Creating notion-optimizer model >> %LOG_FILE%
    ollama create notion-optimizer -f Modelfile
    if errorlevel 1 (
        echo ERROR: Failed to create notion-optimizer model
        echo [%TIMESTAMP%] ERROR: Model creation failed >> %LOG_FILE%
        pause
        exit /b 1
    )
)
...
REM Run content optimization with LLM
echo [%TIMESTAMP%] Running content optimization with LLM >> %LOG_FILE%
python process_updates.py --use-llm
if errorlevel 1 (
    echo ERROR: Failed to run process_updates.py with LLM
    echo [%TIMESTAMP%] ERROR: LLM processing failed >> %LOG_FILE%
    pause
    exit /b 1
)
